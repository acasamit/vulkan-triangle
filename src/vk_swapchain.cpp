...
VkPresentModeKHR VulkanEngine::chooseSwapPresentMode(const std::vector<VkPresentModeKHR>& availablePresentModes) {
	...
	return VK_PRESENT_MODE_FIFO_KHR; // basic v-sync
}

VkExtent2D VulkanEngine::chooseSwapExtent(const VkSurfaceCapabilitiesKHR& capabilities) {
	if (capabilities.currentExtent.width != std::numeric_limits<uint32_t>::max()) {
		return capabilities.currentExtent;
	} else { // we enter this is 1 screen coordinates != 1 pixel
		int width, height;
...
void VulkanEngine::createSwapChain() {
	...
	VkSurfaceFormatKHR surfaceFormat = chooseSwapSurfaceFormat(swapChainSupport.formats); // color depth
	VkPresentModeKHR presentMode = chooseSwapPresentMode(swapChainSupport.presentModes); // conditions for "swapping" images to the screen
	VkExtent2D extent = chooseSwapExtent(swapChainSupport.capabilities); // texture size in swap chain
	...
	vkGetSwapchainImagesKHR(logicalDevice, swapChain, &imageCount, nullptr); // get the real count first
